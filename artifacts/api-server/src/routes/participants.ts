...
router.get("/projects/:id/participants", requireAuth, requireOrganization, async (req, res): Promise<void> => {
...
router.post("/projects/:id/participants", requireAuth, requireOrganization, requireRole(canManageProjects), async (req, res): Promise<void> => {
...
router.patch("/participants/:id", requireAuth, requireOrganization, requireRole(canManageProjects), async (req, res): Promise<void> => {
  const organizationId = getOrganizationId(req)!;
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  ...
  const [project] = await db.select().from(projectsTable).where(and(eq(projectsTable.id, participant.projectId), eq(projectsTable.organizationId, organizationId)));
  if (!project) {
...
router.delete("/participants/:id", requireAuth, requireOrganization, requireRole(canManageProjects), async (req, res): Promise<void> => {
  const userId = getUserId(req)!;
  const organizationId = getOrganizationId(req)!;
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
