    ...
    student: {
      auth: {
        async login(params: {
          ...
          // Map server field names to the response interface.
          // Server returns { id, username, displayName, organizationId, organizationSlug, organizationName, sessionToken }.
          const raw = res.data as unknown as Record<string, unknown>;
          const mapped: StudentLoginResponse = {
            ok: true,
            sessionToken: (raw["sessionToken"] as string) ?? "",
            studentId: (raw["studentId"] as number) ?? (raw["id"] as number) ?? 0,
            displayName: (raw["displayName"] as string) ?? (raw["username"] as string) ?? "",
            organizationId: (raw["organizationId"] as number) ?? 0,
            organizationName: (raw["organizationName"] as string) ?? "",
            organizationSlug: (raw["organizationSlug"] as string) ?? "",
          };
